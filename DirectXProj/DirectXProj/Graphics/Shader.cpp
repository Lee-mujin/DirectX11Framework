...
{
    // Default input layout (Position 3D, UV 2D)
    ...
    return LoadWithLayout(device, vsPath, psPath, layout, numElements, cacheCompiled);
}

bool Shader::LoadWithLayout(ID3D11Device* device, const std::wstring& vsPath, const std::wstring& psPath, const D3D11_INPUT_ELEMENT_DESC* layoutDesc, UINT numElements, bool cacheCompiled)
{
    m_device = device;
    m_vsPath = vsPath;
    m_psPath = psPath;

    m_customLayout.clear();
    if (layoutDesc && numElements > 0)
    {
        m_customLayout.assign(layoutDesc, layoutDesc + numElements);
    }
    ...
    if (!m_customLayout.empty())
    {
        hr = m_device->CreateInputLayout(m_customLayout.data(), (UINT)m_customLayout.size(), vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), &m_inputLayout);
        if (FAILED(hr)) return false;
    }
...
bool Shader::Reload()
{
    // Re-compile and re-create all resources with stored layout.
    return LoadWithLayout(m_device.Get(), m_vsPath, m_psPath, m_customLayout.data(), (UINT)m_customLayout.size(), false);
}
