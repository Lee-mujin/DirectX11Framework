...
{
    // Initialize components that need direct access to Graphics device
    for (auto& go : m_gameObjects)
    {
        ...
        auto terrainRenderers = go->GetComponents<TerrainRenderer>();
        for (auto* terrain : terrainRenderers)
        {
            terrain->Initialize(graphics);
        }

        auto cameras = go->GetComponents<Camera>();
        for (auto* camera : cameras)
        {
            camera->Initialize(graphics);
        }

        auto menus = go->GetComponents<MenuRenderer>();
        for (auto* menu : menus)
        {
